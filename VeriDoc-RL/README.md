...
- `Qwen/Qwen3.5-0.8B` 默认基座
- `vLLM` candidate generation
...
1. `Qwen/Qwen3.5-0.8B` 作为 baseline
...
| 环节 | 当前框架 | 说明 |
| --- | --- | --- |
| baseline candidate 生成 | `vLLM` | 通过 OpenAI-compatible API 提供多候选采样 |
...
- `0.8B` 更适合本地单卡做 smoke 和小规模实验
- `vLLM` 能把 baseline candidate 生成稳定统一到 API 层
...
如果这一步不通，不要继续安装 `torch / vllm / bitsandbytes / verl`。
...
- 直接在配置或命令里写 `Qwen/Qwen3.5-0.8B`
- 第一次真正运行 `vLLM` 或 `transformers.from_pretrained(...)` 时再自动下载

也就是说：

- 开发文档、dry-run、prepare-only 不需要先下载模型
- 真正执行 baseline / SFT / DPO / RL 时，训练机需要能访问这个模型

如果你希望提前缓存到本地路径，也可以先下载到本地目录，再在配置中把模型名替换成目录路径。
...
```bash
ls outputs/pipelines/qwen35_0p8b_mainline
cat outputs/pipelines/qwen35_0p8b_mainline/state.json
cat outputs/pipelines/qwen35_0p8b_mainline/summary.json
```
...
### Q3. DPO 是不是也用同一套 SFT 代码

不是。  
当前 DPO 用的是 `TRL DPOTrainer`。

### Q4. RL 现在是不是 reward model

不是。  
当前默认还是 verifier-based reward。

### Q5. 要不要先手工下载 Qwen/Qwen3.5-0.8B

不需要。  
默认按需下载；只有离线环境才需要提前缓存到本地路径。
