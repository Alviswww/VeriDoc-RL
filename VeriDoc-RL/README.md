...
- 还没有真正接入模型训练代码或 trainer runtime
- `verl.yaml` 是训练配置模板，不保证直接可在你的具体 `verl` 环境原样启动
- 还没有 candidate 采样器、online rollout executor、checkpoint 管理和训练日志聚合
- 还没有真实的 GRPO / RLOO / DPO trainer 封装

也就是说：仓库已经具备数据、verifier、reward、评测、训练语料准备、训练配置生成这些“后训练工程骨架”，但训练执行器本身仍需要后续接入。
...
- 这是 `verl` 风格模板，不是已经对接完真实 runtime 的训练启动器。
- 真正接入 `verl` 时，优先补 `launcher`、`checkpoint`、`logging` 和 `reward callback` 四个层面。
...
10. 用 `generate_training_manifests.py` 生成 `verl` 风格配置模板
...
- training manifest 是模板级，未对接真实分布式训练执行器
...
2. 用 `verl` 补 Phase B/Phase C launcher
3. 增加 reward callback / online verifier bridge
4. 加 checkpoint 目录协议与实验日志聚合
...
- verifier/reward 已通
- 评测与对比已通
- Phase B / C 训练语料与 manifest 脚手架已通

下一步真正缺的是训练执行器接入，而不是再补一层新的 schema 或脚本包装。
