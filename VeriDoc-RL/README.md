...
- Phase B / C `verl` 风格训练 manifest 生成
...
- `phase_b_dpo` 当前仍是语料导出阶段，尚未绑定到可执行 trainer backend

也就是说：仓库已经具备数据、verifier、reward、评测、训练语料准备、训练配置生成，以及 Phase C 的 `verl` launch adapter；真正还缺的是完整训练编排层，而不是底层 reward/评测骨架。
...
8. `src/veridoc_rl/training/manifests.py`
   - 从 experiment matrix 和训练语料路径生成 `verl` 风格 manifest bundle
...
10. `src/veridoc_rl/training/verl_reward.py`
...
- `phase_b_dpo` 当前没有绑定可执行 runtime。
- `phase_c_grpo / phase_c_rloo` 可以继续走下一步 `prepare_verl_runtime.py`。

### 10.11 为 Phase C 准备 verl runtime
...
python scripts/prepare_verl_runtime.py \
  ...
  --run-dir outputs/verl_runs/phase_c_grpo
```
...
- `phase_b_dpo` 会被明确标记为当前 runtime adapter 不支持
...
11. 用 `prepare_verl_runtime.py` 为 `phase_c_grpo / phase_c_rloo` 生成 launch plan
...
- 当前 runtime adapter 只覆盖 `verl` 的 RL phase，未覆盖 DPO
...
2. 给 Phase B 接一个明确的 DPO trainer backend
3. 增加 checkpoint 目录协议与实验日志聚合
4. 加 online rollout executor / candidate sampler
...
下一步真正缺的是把 Phase B trainer 和更完整的训练编排层接上，而不是再补新的 schema。
